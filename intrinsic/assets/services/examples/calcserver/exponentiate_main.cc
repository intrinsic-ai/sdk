...
absl::Status MainImpl() {
  ...
  ExponentiateServiceImpl exponentiate_service;
  ...
  builder.RegisterService(&exponentiate_service);

  // Start the gRPC server.
  std::unique_ptr<::grpc::Server> server(builder.BuildAndStart());
  if (server == nullptr) {
    LOG(FATAL) << "Cannot create Exponentiate Service " << server_address;
  }
  LOG(INFO) << "--------------------------------";
  LOG(INFO) << "-- Exponentiate Service listening on " << server_address;
  LOG(INFO) << "--------------------------------";
