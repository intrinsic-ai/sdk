...
func (r *GenerateCmdRunner) RunE(cmd *cobra.Command, _ []string) error {
	...
	color.C.Blue().Fprintf(cmd.OutOrStdout(), fmt.Sprintf("    inctl recordings get --recording_id %s --with_url --org %s\n", flagBagID, generateParam.GetString(orgutil.KeyOrganization)))
	color.C.Blue().Fprintf(cmd.OutOrStdout(), "\n")
	color.C.Blue().Fprintf(cmd.OutOrStdout(), "  Visualize the recording in your browser:\n")
	color.C.Blue().Fprintf(cmd.OutOrStdout(), fmt.Sprintf("    inctl recordings visualize create --recording_id %s --duration 1h --org %s\n", flagBagID, generateParam.GetString(orgutil.KeyOrganization)))
	fmt.Fprintln(cmd.OutOrStdout(), "")
	return nil
}
