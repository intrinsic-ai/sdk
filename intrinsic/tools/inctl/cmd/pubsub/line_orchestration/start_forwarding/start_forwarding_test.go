...
func TestStartForwarding(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedOutput: []string{fmt.Sprintf("Successfully installed %v", common.ForwardingServiceName)},
			expectErr:      false,
		},
		{
			...
			expectedOutput: []string{
				...
				fmt.Sprintf("Successfully deleted an instance of the %v service", common.ForwardingServiceName),
				fmt.Sprintf("Successfully installed %v", common.ForwardingServiceName),
			},
			expectErr: false,
		},
		{
