...
def create_skill_registry_for_skill_info(
  ...
  Returns:
    The mock skill registry client.
  """
  return create_skill_registry_for_skill_infos([skill_info])


def create_empty_skill_registry() -> skill_registry_client.SkillRegistryClient:
  """Creates a mock client for an empty skill registry.

  Returns:
    The mock skill registry client.
  """
  return create_skill_registry_for_skill_infos([])
...
def create_skill_asset(
  ...
  (
      ...
      parameter_message_full_name,
      return_value_message_full_name,
  ) = _generate_asset_file_descriptor_set(
      parameter_message, return_value_message
  )
  return create_skill_asset_with_file_descriptor_set(
      id,
      version,
      description,
      file_descriptor_set,
...
def create_process_asset(
  ...
  return create_process_asset_with_file_descriptor_set(
      id,
      version,
      description,
      file_descriptor_set,
...
def create_installed_assets(
    assets: list[installed_assets_pb2.InstalledAsset],
  ...
  skill_assets = [
      asset
      for asset in assets
      if asset.metadata.asset_type == asset_type_pb2.AssetType.ASSET_TYPE_SKILL
  ]
  ...
  client.list_all_installed_assets.side_effect = mock_list_all_installed_assets
  return client
...
def create_resource_registry_with_handles(
    handles: list[resource_handle_pb2.ResourceHandle],
  ...
  return resource_registry_client.ResourceRegistryClient(resource_registry_stub)
...
def create_resource_registry_with_single_type_handles(
  ...
  Returns:
    The mock resource registry client.
  """
  return create_resource_registry_with_handles([
      text_format.Parse(
          f"""name: '{name}'
                  resource_data {{
                    key: '{type_name}'
